...
/* ---------------------------------------------------
   UTILS – LANGUAGE DETECTION (INDIA-FIRST)
--------------------------------------------------- */
...
const detectLanguagePreference = (text = "") => {
    ...
    const hindiHints = [
        "haan", "ha", "ji", "boliye", "kya", "kaun",
        "theek", "acha", "bolo", "sun raha"
    ];

    const englishHints = [
        "yes", "speaking", "who is this",
        "tell me", "calling from"
    ];

    let h = hindiHints.filter(k => t.includes(k)).length;
    let e = englishHints.filter(k => t.includes(k)).length;

    if (h > e) return "hinglish";
    if (e > h) return "english";
    return "hinglish";
};
...
app.all('/answer', (req, res) => {
    const callSid = req.body.CallUUID || req.query.CallUUID;
    const leadId = req.query.leadId || req.body.leadId;
    const campaignId = req.query.campaignId || req.body.campaignId;

    const host = req.headers.host;
    const wsUrl =
        `wss://${host}/voice/stream?leadId=${leadId}&campaignId=${campaignId}&callSid=${callSid}`;
...
server.on('upgrade', (req, socket, head) => {
    if (!req.url.startsWith('/voice/stream')) return socket.destroy();
...
/* ---------------------------------------------------
   START REALTIME AI CONNECTION
--------------------------------------------------- */

const startRealtimeWSConnection = async (plivoWS, leadId, campaignId, callSid) => {

    /* ---- STATE PER CALL ---- */
    let preferredLanguage = "hinglish";
    ...
    realtimeWS.on('open', () => {
        realtimeWS.send(JSON.stringify(
            createSessionUpdate(lead, campaign, otherProjects)
        ));
    });
    ...
    realtimeWS.on('message', async msg => {
        ...
        switch (data.type) {
            ...
            case 'conversation.item.input_audio_transcription.completed': {
                ...
                /* ---- FIRST HELLO HANDLING ---- */
                if (!helloHandled) {
                    preferredLanguage = detectLanguagePreference(userText);
                    helloHandled = true;

                    const reply =
                        preferredLanguage === 'english'
                            ? `Reply politely in English.
                               "Hello, good day. This is Govind calling from ${campaign.organization.name}.
                                Am I speaking with ${lead?.name || "you"}?"`
                            : `Reply in natural Hinglish with calm tone.
                               "Hello haan ji, good day 😊
                                Govind bol raha hoon ${campaign.organization.name} se.
                                Kya main ${lead?.name || "aapse"} baat kar raha hoon?"`;

                    realtimeWS.send(JSON.stringify({
                        type: 'response.create',
                        response: { instructions: reply }
                    }));
                }
                break;
            }
            ...
            case 'response.audio_transcript.done':
                conversationTranscript += `AI: ${data.transcript}\n`;
                break;
            ...
            case 'response.function_call_arguments.done':
                if (data.name === 'transfer_call') {
                    const args = JSON.parse(data.arguments);
                    const client = new plivo.Client(PLIVO_AUTH_ID, PLIVO_AUTH_TOKEN);

                    await client.calls.transfer(callSid, {
                        legs: 'aleg',
                        aleg_url: `${NEXT_PUBLIC_SITE_URL}/api/webhooks/plivo/transfer`,
                        aleg_method: 'POST'
                    });

                    await supabase.from('call_logs')
                        .update({ transferred: true, call_status: 'transferred' })
                        .eq('id', callLog.id);
...
wss.on('connection', async (plivoWS, req) => {
    const url = new URL(req.url, `http://${req.headers.host}`);
    const leadId = url.searchParams.get('leadId');
    const campaignId = url.searchParams.get('campaignId');
    const callSid = url.searchParams.get('callSid');

    const realtimeWS = await startRealtimeWSConnection(
        plivoWS,
        leadId,
        campaignId,
        callSid
